...
//define different sizes of segregated free list



/*static const size_t size1 =  min_block_size; //32 bytes

static const size_t size2 =  size1 << 1; //2*32 bytes

static const size_t size3 =  size2 << 1; //4*32 bytes

static const size_t size4 =  size3 << 1; //8*32 bytes

static const size_t size5 =  size4 << 1; //16*32 bytes

static const size_t size6 =  size5 << 1; //32*32 bytes

static const size_t size7 =  size6 << 1; //64*32 bytes

static const size_t size8 =  size7 << 1; //128*32 bytes

static const size_t size9 =  size8 << 1; //256*32 bytes */
...
Note: asize includes the header and footer
so asize should be larger than 16
equal to 16 when size = 0(no allocation, return NULL)

return the index of the corresponding free list pointer
*/
...
static block_t *find_fit(size_t asize) {
    /*block_t *block;
    //first find the bucket from asize
    free_list_start = set[find_bucket(asize)];
 ...
 */
 ...
 /*
 I modified the initialization for segregated free lists
 we have 9 size classes of free lists (32-64, 64-128, 128-256, 256-512, 512-1024, 1024-2048, 2048-4096, 4096-8192, 8192- inf)
 so I declare an array contains 9 pointers, each pointer is the free_list_start of its size classed free list, each in type of  block_t *
 */
bool mm_init(void) {
