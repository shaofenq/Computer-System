 ...
 *
 * @author shaofenq@andrew.cmu.edu
 * Shaofeng Qin, shaofenq
 * TO DO:
 * in this lab we want to build a tiny shell that takes care of both 4 types built-in commands(quit, fg job, bg job, jobs),
 * jobs can be either jid or pid, jobs list all background jobs
 * but also exceve certain files if not built-in
 * There are several functions need to implement by hands: 
 * eval: pass the commmandline argument, make argv list, check if command is built-in or not; do the corresponding jobs, update the job list
 * 
 * implement three types of signal handlers:
 * (1)sigchld handler: Handles SIGCHLD signals.
 (2)sigint handler: Handles SIGINT signals (sent by Ctrl-C).
 (3) sigtstp handler: Handles SIGTSTP signals (sent by Ctrl-Z).
 */
...
void do_builtin(int argc, char** argv)
{
    ...
    job_state state; // job state of the job to handle
    //int olderrno = errno;
    sigfillset(&mask_all);
    ...
    // if it is a jid
    else if ((arg[0] == '%') && (isdigit(arg[1]) == true))
    {
        sigprocmask(SIG_BLOCK, &mask_all, &prev_all); //block all signals
        printf("arg + 1 IS %s\n", arg + 1);
...
void eval(const char *cmdline) {
    ...
    */
    ...
    if (parse_result == PARSELINE_ERROR || parse_result == PARSELINE_EMPTY) {
        ...
        // this codeskeleton is from our text book page 757
