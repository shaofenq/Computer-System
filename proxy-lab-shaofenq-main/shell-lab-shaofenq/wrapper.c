...
static void init_signal_wrapping() {
 ...
 */
void sa_sigaction_wrapper(int signum, siginfo_t * siginfo, void * context) {
    // save errno
    int olderrno = errno;
    ...
    if (errno != ERRNO_SENTINEL) {
        ...
        sio_printf("         You must save and restore errno in signal handlers!\n");
    }
