...
char *cleanup_args[4] = {"/bin/sh", "-c", NULL, NULL};
...
void graceful_cleanup_sig_handler(int sig) {
    // block signal
    sigset_t mask_all, prev_all;
...
int main(int argc, char **argv) {
    ...
    if (mkdir(RUNTRACE_TMP_FOLDER, 0700) != 0) {
        if(errno == EEXIST) {
            fprintf(stderr,
                    "RUNTRACE ERROR: Temporary directory (%1$s) already exists.\n"
                    "  Make sure you aren't running the driver concurrently.\n"
                    "  Then run 'rm -rf %1$s/', and retry.\n", RUNTRACE_TMP_FOLDER);
            _exit(1);
        } else {
    ...
    const char *fmt = "/bin/kill -9 $(/bin/ps -s %d -o pid=) > /dev/null 2>&1;"
                      " rm -Rf " RUNTRACE_TMP_FOLDER;
    size_t sz = snprintf(NULL, 0, fmt, child_pid);
    cleanup_args[2] = calloc(sz + 1, sizeof(char));
    if (!cleanup_args[2]) {
        sio_eprintf("calloc error");
        _exit(1);
    }
    snprintf(cleanup_args[2], sz + 1, fmt, child_pid);
    ...
    while (fgets_result) {
        ...
        /* SHELLWAIT command */
            ...
            if (readable(shellsyncfd[0], DRIVER_TIMEOUT) == 0) {
                printf("%s: Runtrace timed out waiting for sync from the shell\n",
                       tracefile);
                if (sigprocmask(SIG_BLOCK, &mask_all, &prev_all) < 0) {
                    sio_eprintf("sigprocmask error in main\n");
                    _exit(1);
         ...
         * sends fg/bg $(pgrep -s session_id name) to the shell */
            ...
            if (snprintf(command, sizeof(command), "%s %s",
                         builtin, name) >= sizeof(command)) {
                perror("snprintf");
                exit(1);
            }
...
int next_prompt(void) {
    ...
    while ((n = select(maxfd + 1, &readable_set, NULL, NULL, &tv)) > 0) {
        if (FD_ISSET(shellsyncfd[0], &readable_set)) {
            ...
            if (verbose) {
                printf("runtrace: received sync from shell waiting for prompt\n");
            }
